...
# 2) EDA  : Exploaratory Data Analysis
```{r summary}
summary(reg_dfw)
```
...
mod1_int <-lmer(yield_mgha ~ 1 + (1 | rep), 
                data = reg_dfw) # 1 means yield is not a function of anything, lm is used for fixed effect only models while lmer is used for random effect in our case it is coming from a block (rep)
...
#bootstrap means sampling with replacement. It means the same data point can be sampled more than ones. Instead of running one, you may run a thousand.
...
nd <- nd %>%
  mutate (mod1_yield_mgha = predict(mod1_int,
                                    nd,
                                    re.form = NA
                                    )) #re.form means do not post the random effect (ignore) as part of the model when giving me the mean prediction.

# Creating function to bootstrap, mod can be called anything
...
mod1_int_boots <- bootMer(mod1_int,
  ...
  confint() %>%
  as.data.frame() %>%
  rename(mod1_int_lcl = `2.5 %`, mod1_int_ucl = `97.5 %`) #bootMer is used for model of type lmer for mixed and random effect, nsim is the number of bootstrap samples you want to take
...
```
sr_ksha above has a slope attached to it under the fixed effects outcome.
...
Residuals are where the model of assumptions are based not the raw data so check your assumptions on the residuals not the raw data.
...
mod3_quad <- lmer(yield_mgha ~ sr_ksha +
                    I(sr_ksha^2) + (1|rep),
                  data = reg_dfw
                    ) #allows to run mixed effect models
...
Which one should we chose for finding the optimum, or predicting new data? Quadratic 
...
mod4_linp_aug
```
.resid is the total distance from the point 
### Within-group errors are iid ~ N(0, var2)
...
```
nd is where we saved all the fitted models
...
```
My note: BIC is more conservative. The lower the better for all the matrix above. d in dAIC stand for delta.
